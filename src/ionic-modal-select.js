...
angular.module('ionic-modal-select', [])
    ...
    .service('modalSelectDelegate', modalSelectDelegate);
...
function modalSelect($ionicModal, $timeout, $filter, $parse, $templateCache, modalSelectDelegate) {
    ...
    return {
        ...
        link: function (scope, iElement, iAttrs, ngModelController, transclude) {
            ...
            if (iAttrs.handlerId)
                modalSelectDelegate._addScope(iAttrs.handlerId, scope);
            ...
            scope.$on('$destroy', function(){
                ...
                scope.modal.remove();
                if (iAttrs.handlerId)
                    modalSelectDelegate._removeScope(iAttrs.handlerId, scope);
            });
            ...
            iElement.on('click', scope.showModal);
...
function modalSelectDelegate() {
    let _scopes = {};

    this._addScope = function (handlerId, scope) {
        _scopes[handlerId] = scope;
    };

    this._removeScope = function (handlerId) {
        delete _scopes[handlerId];
    };

    this.getHandler = function (handlerId) {
        if (!_scopes[handlerId])
            return null;

        return new modalSelectDelegateHandler(_scopes[handlerId]);
    };

    function modalSelectDelegateHandler(scope) {
        this.reset = function () {
            if (scope.isMultiple)
                scope.unsetValues();
            else
